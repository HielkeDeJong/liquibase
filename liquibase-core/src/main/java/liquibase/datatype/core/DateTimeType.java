...
public class DateTimeType extends LiquibaseDataType {
	...
	public DatabaseDataType toDatabaseDataType(Database database) {
		if (database instanceof DerbyDatabase || database instanceof FirebirdDatabase || database instanceof H2Database
				|| database instanceof HsqlDatabase || database instanceof OracleDatabase) {
		...
		if (database instanceof MySQLDatabase) {
			...
			try {
				supportsParameters = database.getDatabaseMajorVersion() >= 5 && database.getDatabaseMinorVersion() >= 6
	...
	public Object sqlToObject(String value, Database database) {
		...
		try {
			...
			if (database instanceof OracleDatabase
					&& value.matches("to_date\\('\\d+\\-\\d+\\-\\d+ \\d+:\\d+:\\d+', 'YYYY\\-MM\\-DD HH24:MI:SS'\\)")) {
	...
	protected DateFormat getDateTimeFormat(Database database) {
		if (database instanceof MySQLDatabase) {
			return new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); // no ms in
																// mysql
		}
		if (database instanceof MSSQLDatabase) {
			return new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS"); // no ms
																		// in
																		// mysql
		}
	...
	// oracle
	// @Override
	// public Object convertDatabaseValueToObject(Object defaultValue, int
	// dataType, int columnSize, int decimalDigits, Database database) throws
	// ParseException {
	// if (defaultValue != null) {
	// if (defaultValue instanceof String) {
	// if (dataType == Types.DATE || dataType == Types.TIME || dataType ==
	// Types.TIMESTAMP) {
	// if (((String) defaultValue).indexOf("YYYY-MM-DD HH") > 0) {
	// defaultValue = ((String) defaultValue).replaceFirst("^to_date\\('",
	// "").replaceFirst("', 'YYYY-MM-DD HH24:MI:SS'\\)$", "");
	...
	// //sometimes oracle adds an extra space after the trailing ' (see
	// http://sourceforge.net/tracker/index.php?func=detail&aid=1824663&group_id=187970&atid=923443).
	// }
	// }
	// return super.convertDatabaseValueToObject(defaultValue, dataType,
